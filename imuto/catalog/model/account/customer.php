...
class ModelAccountCustomer extends Model {
	...
	public function addcode($data) {
		$this->db->query("INSERT INTO " .DB_PREFIX . "email_code SET email = '" . $this->db->escape($data['email']) . "', code = '" . (int)$data['code'] . "', date_added = NOW()");
		return 1;
	}
	public function getcode($data) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "email_code WHERE email = '" . $this->db->escape($data) . "' ORDER BY id desc");
		return $query->row;
	}
	public function delcode($data) {
		$query = $this->db->query("DELETE FROM " . DB_PREFIX . "email_code WHERE email = '" . $this->db->escape($data) . "'");
	}
	public function editCustomer($customer_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', newsletter = '" . (isset($data['newsletter']) ? (int)$data['newsletter'] : 0) . "', telephone = '" . $this->db->escape($data['telephone']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? json_encode($data['custom_field']['account']) : '') . "', date_birth = '". $this->db->escape($data['date_birth']) . "', country = '". $this->db->escape($data['country']) . "' WHERE customer_id = '" . (int)$customer_id . "'");
	}
