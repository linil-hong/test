...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ($this->request->get['user_token'] == $this->session->data['user_token'])) {
			...
			if ($this->user->hasPermission('access', 'catalog/best_seller')) {
				$catalog[] = array(
					'name'	   => $this->language->get('text_best_seller'),
					'href'     => $this->url->link('catalog/best_seller', 'user_token=' . $this->session->data['user_token'], true),
					'children' => array()
				);
			}

			if ($this->user->hasPermission('access', 'catalog/discount_activity')) {
				$catalog[] = array(
					'name'	   => $this->language->get('text_discount_activity'),
					'href'     => $this->url->link('catalog/discount_activity', 'user_token=' . $this->session->data['user_token'], true),
					'children' => array()
				);
			}

			if ($this->user->hasPermission('access', 'catalog/new_products')) {
				$catalog[] = array(
					'name'	   => $this->language->get('text_new_products'),
			...
			//2019-5-13  邮件群发
			if ($this->user->hasPermission('access', 'marketing/group')) {
				$marketing[] = array(
			...
			if ($this->user->hasPermission('access', 'marketing/subscribe')) {
				$marketing[] = array(
					'name'	   => $this->language->get('text_subscribe'),
					'href'     => $this->url->link('marketing/subscribe', 'user_token=' . $this->session->data['user_token'], true),
					'children' => array()
				);
			}
