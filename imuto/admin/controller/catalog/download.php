...
class ControllerCatalogDownload extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['content'])) {
			...
			$data['content'] = $download_info['content'];
		} else {
			$data['content'] = '';
		}
		...
		} elseif (!empty($download_info) && is_file(DIR_IMAGE . $download_info['image'])) {
			$data['thumb'] = $this->model_tool_image->resize($download_info['image'], 100, 100);
		} else {
