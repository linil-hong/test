...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['color_code'])) {
			...
			$data['color_code'] = $product_info['color_code'];
		} else {
			$data['color_code'] = '';
		}

		if (isset($this->request->post['is_main_color'])) {
			$data['is_main_color'] = $this->request->post['is_main_color'];
		} elseif (!empty($product_info)) {
			$data['is_main_color'] = $product_info['is_main_color'];
		} else {
			$data['is_main_color'] = 1;
		}
		...
		if (isset($this->request->post['jp_price'])) {
			$data['jp_price'] = $this->request->post['jp_price'];
		} elseif (!empty($product_info)) {
			$data['jp_price'] = $product_info['jp_price'];
		} else {
			$data['jp_price'] = '';
		}		
		if (isset($this->request->post['ca_price'])) {
			$data['ca_price'] = $this->request->post['ca_price'];
		} elseif (!empty($product_info)) {
			$data['ca_price'] = $product_info['ca_price'];
		} else {
			$data['ca_price'] = '';
		}		
		if (isset($this->request->post['it_price'])) {
			$data['it_price'] = $this->request->post['it_price'];
		} elseif (!empty($product_info)) {
			$data['it_price'] = $product_info['it_price'];
		} else {
			$data['it_price'] = '';
		}		
		if (isset($this->request->post['fr_price'])) {
			$data['fr_price'] = $this->request->post['fr_price'];
		} elseif (!empty($product_info)) {
			$data['fr_price'] = $product_info['fr_price'];
		} else {
			$data['fr_price'] = '';
		}		
		if (isset($this->request->post['uk_price'])) {
			$data['uk_price'] = $this->request->post['uk_price'];
		} elseif (!empty($product_info)) {
			$data['uk_price'] = $product_info['uk_price'];
		} else {
			$data['uk_price'] = '';
		}
		if (isset($this->request->post['au_price'])) {
			$data['au_price'] = $this->request->post['au_price'];
		} elseif (!empty($product_info)) {
			$data['au_price'] = $product_info['au_price'];
		} else {
			$data['au_price'] = '';
		}
		...
		$filter_country_attributes_data = array(
			'filter_attribute_group_id' => 7, //取shopLink这个属性组的ID
		);
		$country_attributes = $this->model_catalog_attribute->getAttributes($filter_country_attributes_data);
		$data['country_attributes'] = $country_attributes;

		if (isset($this->request->post['product_country_prices'])) {
			$product_country_prices = $this->request->post['product_country_prices'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_country_prices = $this->model_catalog_product->getProductCountryPrices($this->request->get['product_id']);
		} else {
			$product_country_prices = array();
		}

		$data['product_country_prices'] = array();

		foreach ($product_country_prices as $product_country_price) {
			$data['product_country_prices'][] = array(
				'country_attribute_id' => $product_country_price['country_attribute_id'],
				'original_price'       => $product_country_price['original_price'],
				'special_price'        => $product_country_price['special_price'],
				'sort_order'           => $product_country_price['sort_order'],
				'symbol'           	   => $product_country_price['symbol'],
				'date_start'			=> $product_country_price['date_start'],
				'time_start'			=> $product_country_price['time_start'],
				'date_end'			=> $product_country_price['date_end'],
				'time_end'			=> $product_country_price['time_end'],
			);
		}
		
		//2019-5-8 : 货币符列
		$symbol_all = array('$','C$','¥',"₣","€","₳","₰");
		$data['symbol_all'] = $symbol_all;
		...
		//2019-5-24 Japan Images
		if (isset($this->request->post['product_jpimg'])) {
			$product_jpimgs = $this->request->post['product_jpimg'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_jpimgs = $this->model_catalog_product->getProductJpimgs($this->request->get['product_id']);
		} else {
			$product_jpimgs = array();
		}

		$data['product_jpimgs'] = array();

		foreach ($product_jpimgs as $product_jpimg) {
			if (is_file(DIR_IMAGE . $product_jpimg['image'])) {
				$jpimg = $product_jpimg['image'];
				$thumb = $product_jpimg['image'];
			} else {
				$jpimg = '';
				$thumb = 'no_image.png';
			}
			...
			$data['product_jpimgs'][] = array(
				'image'      => $jpimg,
				'thumb'      => $this->model_tool_image->resize($thumb, 100, 100),
				'sort_order' => $product_jpimg['sort_order']
			);
		}
	...
	protected function validateForm() {
		...
		//2019-3-4: 添加十六进制颜色码字段
		if((utf8_strlen($this->request->post['color_code']) < 1) || (utf8_strlen($this->request->post['color_code']) > 7)) {
			$this->error['color_code'] = $this->language->get('error_color_code');
		}
