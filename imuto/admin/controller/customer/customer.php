...
class ControllerCustomerCustomer extends Controller {
	...
	public function phpexcel()
	{
		$this->load->language('customer/customer');
		$this->load->model('customer/customer');
			require "C:/imuto_website/imuto/imbackstage/PHPExcel/PHPExcel.php";
		$objPHPExcel = new PHPExcel();
		$objSheet = $objPHPExcel->getActiveSheet();
		$objSheet->setTitle('message');
		$objSheet->setCellValue("A1","customer_id")->setCellValue("B1","firstname")->setCellValue("C1","lastname")->setCellValue("D1","email")->setCellValue("E1","telephone")->setCellValue("F1","ip")->setCellValue("G1","country")->setCellValue("H1","birth");

		$customers = $this->model_customer_customer->putexcel();
		$i = 2;
		foreach($customers as $value)
		{
			$value['birth'] = $value['date_birth'];
			$objSheet->setCellValue("A".$i,$value['customer_id'])->setCellValue("B".$i,$value['firstname'])->setCellValue("C".$i,$value['lastname'])->setCellValue("D".$i,$value['email'])->setCellValue("E".$i,$value['telephone'])->setCellValue("F".$i,$value['ip'])->setCellValue("G".$i,$value['country'])->setCellValue("H".$i,$value['birth']);
			$i++;
		}
		...
		$objWirter = PHPExcel_IOFactory::createWriter($objPHPExcel,"Excel2007");
		$objWirter->save("php://output");
	...
	protected function getList() {
		...
		$data['phpexcel'] = $this->url->link('customer/customer/phpexcel', 'user_token=' . $this->session->data['user_token'] . $url, true);
	...
	protected function getForm() {
		...
		if (isset($this->request->post['date_birth'])) {
			...
			$data['date_birth'] = $customer_info['date_birth'];
		} else {
			$data['date_birth'] = '';
		}
		if (isset($this->request->post['country'])) {
		...
		$country_all = array('America','Canada','China','Japan','Britain','Germany','France','India','Italy','Australia','The United Arab Emirates');
